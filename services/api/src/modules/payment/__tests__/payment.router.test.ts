...
describe('routePaymentProvider', () => {
  ...
  it('routes VISA to the global card provider', () => {
    expect(routePaymentProvider('ID', 'VISA', providers)).toBe(
      providers.globalCard,
    );
  });

  it('routes MASTERCARD to the global card provider', () => {
    expect(routePaymentProvider('ID', 'MASTERCARD', providers)).toBe(
      providers.globalCard,
    );
  });

  it('routes PAYPAL to the standalone PayPal provider', () => {
    expect(routePaymentProvider('ID', 'PAYPAL', providers)).toBe(
      providers.paypal,
    );
  });

  it('routes China Alipay to Xendit', () => {
    expect(routePaymentProvider('CN', 'ALIPAY', providers)).toBe(
      providers.xendit,
    );
  });

  it('routes China WeChat Pay to Xendit', () => {
    expect(routePaymentProvider('CN', 'WECHAT_PAY', providers)).toBe(
      providers.xendit,
    );
  });

  it('routes Russia SBP to the Russia provider', () => {
    expect(routePaymentProvider('RU', 'SBP', providers)).toBe(
      providers.russia,
    );
  });

  it('routes Russia MIR to the Russia provider', () => {
    expect(routePaymentProvider('RU', 'MIR', providers)).toBe(
      providers.russia,
    );
  });

  it('routes Russia CRYPTO to the Russia provider', () => {
    expect(routePaymentProvider('RU', 'CRYPTO', providers)).toBe(
      providers.russia,
    );
  });
