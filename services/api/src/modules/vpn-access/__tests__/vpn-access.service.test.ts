...
import {
  generateOwnedVPNAccess,
  generateVPNAccess,
} from '../vpn-access.service.js';
...
describe('VPN Access Service', () => {
  beforeEach(() => {
    ...
    findActiveVPNNodeMock.mockResolvedValue(
      buildVPNNode(),
    );

    createVPNAccessMock.mockResolvedValue(
      buildVPNAccess(true),
    );
  ...
  describe('generateVPNAccess', () => {
    it('rejects when the license does not exist', async () => {
      ...
      await expect(
        generateVPNAccess('missing-license'),
    ...
    it('rejects non-WireGuard products', async () => {
      findVPNAccessOwnershipMock.mockResolvedValue(
        buildOwnership('user-1', 'GENERAL-PRO'),
      );
    ...
    it('reactivates an existing inactive VPN access', async () => {
      ...
      findVPNAccessByLicenseMock.mockResolvedValue(
        existing,
      );
    ...
    it('reuses an existing active VPN access', async () => {
      ...
      const result = await generateVPNAccess(
        'license-1',
      );
  ...
  describe('generateOwnedVPNAccess', () => {
    ...
    it('generates VPN access for the license owner', async () => {
      ...
      const result = await generateOwnedVPNAccess(
        'license-1',
        'user-1',
      );
