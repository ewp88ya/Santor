...
export async function expireSubscriptions() {
  ...
  for (const subscription of subscriptions) {
    if (subscription.autoDebitEnabled && (!subscription.gracePeriodEnd || subscription.gracePeriodEnd > now)) continue;

    const didExpire = await prisma.$transaction(async (tx) => {
      const current = await tx.subscription.findUnique({ where: { id: subscription.id } });
      if (!current || current.status !== 'active' || !current.endDate || current.endDate >= now) return false;
      if (current.autoDebitEnabled && (!current.gracePeriodEnd || current.gracePeriodEnd > now)) return false;
